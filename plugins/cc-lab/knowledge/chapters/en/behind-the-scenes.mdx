...
**Demonstration.** The strongest evidence that the patterns in [Chapter 4](/en/lab/iteration-and-control) and [Chapter 7](/en/lab/compound-engineering) — plan mode, proof slices, the compound loop, the harness-first posture — actually scale is a lab built using exactly those patterns. This chapter is the receipt.

## Practice: render your own dashboard

A good final exercise: turn what you just read into a dashboard of your own usage. The patterns from Chapters 4 and 7 carry: narrow scope, plan before work, proof slice before batch.

**Try this plan prompt** — one scoped task, one concrete output:

<Prompt>/plan: Write me a Python script that reads the last 30 days of my Claude Code session jsonl files from ~/.claude/projects/ and renders a single-page HTML dashboard. Per session: duration, turn count, subagent count, token totals, API-rate cost. Use the published pricing at platform.claude.com for current rates. One HTML file. No dependencies beyond the Python stdlib.</Prompt>
