...
**Skill** je balík instrukcí + volitelných skriptů, které si Claude načte podle potřeby. Doménový playbook — bydlí v `~/.claude/skills/`, aktivuje se sám, když situace sedí. Skilly se dají stohovat v jednom promptu: `/skill-a /skill-b udělej XYZ` načte všechny vedoucí skilly, až pět.
...
Jeden vestavěný skill, který stojí za to znát: `/dataviz` — design grafů a dashboardů s validátorem barevné palety, který rovnou spustíš. Spusť ho ještě před tím, než začneš psát kód grafů.
...
**Důvěra v workspace.** `claude mcp list` a `claude mcp get` teď nespouštějí `.mcp.json` servery, které si repo samo schválilo commitem do `.claude/settings.json`. V nedůvěryhodném workspace se takové servery zobrazí jako `⏸ Pending approval` — explicitně je schválíš, než se připojí.
...
Můžeš je volat přímo, nebo to nech Claude rozhodnout. Od verze 2.1.198 subagenti běží v pozadí ve výchozím nastavení — Claude v hlavní session pracuje dál, zatímco subagenti běží, a dostane oznámení, až doběhnou.

### Zkus jednoho: vestavěný Explore agent

Vestavěný. Read-only, rychlý — dědí model tvé session, maximálně Opus — ideální na otázky „kde je X?" ve velkém codebase.
...
Pro opakující se delegování — třeba reviewera, co vždycky aplikuje vaše týmové standardy — řekni Claudovi přímo:

<Prompt>Vytvoř subagenta pro code review, rozsah projekt. Jen čtení — žádné editace, žádný shell.</Prompt>

Claude vygeneruje definici a zapíše ji do `.claude/agents/`. Soubory v `.claude/agents/` si taky můžeš editovat ručně.
...
**Extended thinking.** Subagenti a context compaction dědí konfiguraci extended thinking pro session, což zlepšuje kvalitu výstupu delegovaných úkolů.
