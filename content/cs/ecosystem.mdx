...
Nemáš na stroji git ani npm? Zdroj pluginů `archive` nainstaluje rovnou ze zipu přes HTTPS — s volitelným SHA-256 pinováním, když chceš zamknout přesný soubor.
...
## Zprávy mezi sessions

Claude Code session si teď můžou psát napřímo — na libovolném z tvých strojů, macOS i Linux. `ListAgents` ukáže, co je dosažitelné — tvoje Remote Control session na jiných strojích, adresovatelné jménem. `SendMessage` doručí zprávu kterékoliv z nich a teď umí konverzaci i sám začít, ne jen odpovídat, až tě někdo osloví první. O bezpečnostní pojistku se starají dvě nastavení dohromady: `crossSessionInbound` a `dialogExpiry` zprávu pozdrží na tvoje schválení, když míří do session s obejitými oprávněními; všechno ostatní doručí rovnou.

→ [Dokumentace ke zprávám mezi sessions](https://code.claude.com/docs/en/cross-session-messaging)

## Remote a mobil — Claude mimo tvůj stůl

Tři různé produkty, které si lidé pletou. Remote Control a Claude Code na Webu vyžadují plán Pro nebo vyšší — pouhé API klíče jsou mimo hru. Self-hosted environments jsou jen pro Team a Enterprise.
...
### Self-hosted environments — tvoje vlastní infrastruktura

Třetí možnost, na plánech Team a Enterprise: `claude self-hosted-runner` udělá z tvých vlastních strojů nebo kontejnerů místo, kde běží Claude Code web, mobil i desktop session.

→ [Dokumentace k self-hosted environments](https://code.claude.com/docs/en/self-hosted-environments)
