...
**Skládání invokací.** `/skill-a /skill-b udělej XYZ` načte všechny skilly na začátku příkazu (až 5), pak spustí úkol se všemi aktivními. Hodí se, jestli dva skilly přirozeně spolupracují — třeba research skill a formátovací skill v jediném příkazu.
...
**Automatické přesunutí na pozadí.** Volání MCP nástrojů, která jedou déle než 2 minuty, se přesunou na pozadí automaticky — session zůstane použitelná. Práh nastav nebo zakáž přes `CLAUDE_CODE_MCP_AUTO_BACKGROUND_MS`.
...
Vestavěný. Read-only. Přebírá model tvé session, zastropovaný na Opusu — Haiku je pryč. Ideální na otázky „kde je X?" ve velkém codebase.
...
Pro opakujícího se specialistu — třeba reviewera, co vždycky aplikuje vaše týmové standardy — požádej Claude, ať ti definici subagenta sepíše, nebo upravuj `.claude/agents/my-agent.md` přímo. Nastav rozsah na Personal nebo Project, omez nástroje, ulož. Agent pak funguje přes `@my-agent` nebo přirozeným jazykem.
...
**Běh na pozadí jako výchozí.** Subagenti teď jedou na pozadí — Claude pracuje dál, zatímco jedou, a dostane oznámení, až skončí. Subagent dělající kód v izolovaném worktree commitne, pushne a otevře draft PR místo toho, aby se zeptal.

**`/fork` a `/subtask`.** `/fork` zkopíruje aktuální konverzaci do nové background session (vlastní řádek v `claude agents`) — v originálu pracuješ dál. `/subtask` je nové jméno pro dřívějšího in-session subagenta, kterého `/fork` spouštěl.
