...
Do promptu napiš `@` a jméno jiné session Claude Code — Claude ji pak sám osloví přes `SendMessage`, volání skládat ručně nemusíš.
...
**Fork místo studeného startu.** `subagent_type: "fork"` spustí subagenta, který zdědí celou konverzaci i tvou prompt cache, místo aby začínal s prázdným kontextem — sáhni po něm, když úkol subagenta stojí na tom, co jste s Claudem už v téhle session vyřešili, ne na novém průzkumu.
