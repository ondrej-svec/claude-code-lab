...
Pro opakující se delegování — třeba reviewera, co vždycky aplikuje vaše týmové standardy — požádej Clauda, ať jednoho napíše, nebo vytvoř definici přímo:

```
claude "Vytvořit code-reviewer subagenta, který hlídá naše týmové konvence"
```

Claude zapíše definici do `.claude/agents/<jméno>.md`. Nastav scope (Personal nebo Project), omez nástroje. Agent pak je dostupný přes `@<jméno>` nebo přirozeným jazykem. Wizard pro vytváření agentů v `/agents` byl odstraněn ve v2.1.198 — požádej Clauda o vytvoření nebo správu subagentů, nebo edituj soubory v `.claude/agents/` přímo.
...
**Pozadí jako výchozí.** Od v2.1.198 subagenti jedou na pozadí — Claude pokračuje v práci v hlavní konverzaci, zatímco subagent plní svůj úkol, a dozvídá se, až je hotovo.
