...
Background session teď svou práci samy commitují a pushují, aby se neztratila, a draft PR otevřou jen tehdy, když si to úkol fakticky žádá — řídí se přitom tvými git instrukcemi z `CLAUDE.md` stejně jako interaktivní session. Každý běh na pozadí na konci nahlásí, kde práce přistála, takže nehádáš, na jaké větvi hledat.
...
Pluginy nainstalované přes `/plugin` se teď aktivují hned, jakmile je to bezpečné — `/reload-plugins` už není potřeba pokaždé. Příkaz si nech v záloze pro případy, kdy ho ještě potřebuješ.
