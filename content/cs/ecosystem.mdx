...
**Vrstvená volání skillů.** Před prompt lze řetězit až pět skillů: `/skill-a /skill-b udělej XYZ` načte oba, než Claude dostane zadání. Hodí se na úkoly přes víc oblastí — `/security-review /code-review refaktor auth modulu` zapojí obě perspektivy najednou.
...
Od verze 2.1.198 **subagenti běží na pozadí ve výchozím stavu** — Claude mezitím pokračuje v práci a výsledky dostane, až skončí.

### Zkus jednoho: vestavěný Explore agent

Vestavěný. Read-only, přebírá model session (maximálně Opus) — ideální na otázky „kde je X?" ve velkém codebase.
...
Pro opakující se delegování — třeba reviewera, co vždycky aplikuje vaše týmové standardy — popros Claude přímo:

<Prompt>Vytvoř subagenta code-reviewer pro tento projekt. Ať aplikuje naše týmové standardy při review.</Prompt>

Claude napíše definici do `.claude/agents/my-agent.md`. Agent je pak dostupný přes `@my-agent` nebo přirozeným jazykem. Soubor uprav přímo — je to jen markdown — jestli chceš doladit nástroje, scope nebo instrukce.
