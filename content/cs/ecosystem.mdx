...
**Zdroj marketplace může být i `command`.** Lokální příkaz — třeba IDE — může sám vypsat cestu k&nbsp;adresáři pluginu; Claude Code ji každou session znovu vyřeší a&nbsp;změny se projeví bez restartu. `mode: "link"` použije ten adresář přímo místo kopírování.
...
**Forkování dědí kontext ve výchozím stavu.** Subagent s `subagent_type: "fork"` teď automaticky zdědí celou konverzaci a&nbsp;prompt cache místo studeného startu — nejrychlejší předání práce, kdykoliv subagent potřebuje všechno, co už máš načtené.
