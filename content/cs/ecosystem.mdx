...
**Skládání skillů.** Z jednoho promptu se dá načíst víc skillů najednou: `/skill-a /skill-b udělej XYZ` aktivuje oba (až pět vedoucích skillů). Šikovné pro kombinaci specializovaného skillu s obecným bez nutnosti upravovat ani jeden.
...
Vestavěný. Read-only, ideální na otázky „kde je X?" ve velkém codebase. Zdědí model tvé session (maximálně Opus).
...
Pro opakující se delegování — třeba reviewera, co vždycky aplikuje vaše týmové standardy — řekni to Claudovi nebo si soubor definice hodíš rovnou do `.claude/agents/`. Průvodce `/agents` zmizel; přirozený jazyk a přímá editace souborů jsou teď dvě cesty.

<Prompt>Vytvoř projektový subagent code-reviewer, co kontroluje naše konvence z CLAUDE.md před každým code review.</Prompt>

Claude vygeneruje definici agenta a uloží ji. Tvůj agent je dostupný přes `@code-reviewer` nebo přirozeným jazykem.
...
**Pozadí jako výchozí.** Subagenti teď běží na pozadí — Claude pracuje dál, zatímco subagent jede, a dostane oznámení, až skončí. Agenti spuštění z pohledu agentů (`claude agents`) při práci ve worktree automaticky commitnou, pushnou a otevřou draft PR, místo aby se zastavili a ptali.
