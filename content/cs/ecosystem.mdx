...
Můžeš je volat přímo, nebo to nech Claude rozhodnout. Subagenti teď běží na pozadí ve výchozím nastavení. Claude pokračuje v práci na hlavním úkolu, zatímco oni běží, a dostane notifikaci, až skončí.
...
Pro opakující se delegování — třeba reviewera, co vždycky aplikuje vaše týmové standardy — jednoduše řekni Claudovi:

<Prompt>Vytvoř subagenta jménem code-reviewer, který čte codebase a kontroluje změny podle týmových konvencí. Omez ho jen na read-only nástroje.</Prompt>

Claude vygeneruje definici a zapíše ji do `.claude/agents/code-reviewer.md`. Nastavíš scope (Personal nebo Project), omezíš nástroje přímo v souboru — a agent je dostupný přes `@code-reviewer` nebo přirozeným jazykem.
