...
Session si teď umí i navzájem posílat zprávy — na libovolném z tvých strojů, na macOS a Linuxu. `ListAgents` vypíše, co je dosažitelné; `SendMessage` pošle zprávu rovnou jedné z nich. To teď jde dál: `SendMessage` umí začít konverzaci s Remote Control session na jiném stroji podle jména, místo aby jen odpovídal, až se ozve ona první. `ListAgents` takovou session ukáže jako `name [ref]`. Hodí se, když background session na laptopu najde něco, co potřebuje vědět ta na stole, dřív než se dostaneš k jedné nebo druhé klávesnici.
...
Nemáš ke zdroji pluginu přístup přes git nebo npm? Zdroj `archive` nainstaluje plugin rovnou ze zipu přes HTTPS, s volitelným SHA-256 pinningem, ať víš přesně, co pouštíš.
