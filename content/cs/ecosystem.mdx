...
**Nedůvěryhodné workspaces (v2.1.196+).** Naklonuješ-li repo s commitnutým `.claude/settings.json`, který si self-approvuje vlastní `.mcp.json` servery, tyhle servery se teď nespustí automaticky. `claude mcp list` je zobrazí jako `⏸ Pending approval` — než se připojí, musíš je výslovně schválit.
