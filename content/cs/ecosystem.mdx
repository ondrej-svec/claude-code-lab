...
Vestavěný. Read-only, běží na modelu session (maximálně Opus). Ideální na otázky „kde je X?" ve velkém codebase.
...
Pro opakující se delegování — třeba reviewera, co vždycky aplikuje vaše týmové standardy — řekni Claudovi přímo v session nebo si upravuj `.claude/agents/` ručně.

<Prompt>Vytvoř agenta "my-reviewer", který na každý diff aplikuje naše týmové bezpečnostní standardy. Scope na tento projekt.</Prompt>

Claude zapíše definici do `.claude/agents/my-reviewer.md`. Agent je dostupný přes `@my-reviewer` nebo přirozeným jazykem.

<Screenshot
  src="/screenshots/ch5-agents-ui.png"
  alt="UI /agents — projektoví, osobní a library subagenti s popisy"
  caption="`/agents` vypíše všechny nakonfigurované agenty — agenti žijí jako `.md` soubory v `.claude/agents/`."
/>
