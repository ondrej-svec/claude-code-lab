...
Třetí zdroj marketplace: `command`. Nasměruj záznam na lokální příkaz — třeba tvoje IDE — a&nbsp;ten vypíše cestu k&nbsp;adresáři pluginu; Claude Code tu cestu znovu vyhodnotí každou session, takže se změny projeví bez restartu. S&nbsp;`mode: "link"` se adresář použije přímo na místě, místo aby se kopíroval.
...
Vracíš se k session později? `claude remote-control --continue` obnoví tvoji poslední Remote Control session.
