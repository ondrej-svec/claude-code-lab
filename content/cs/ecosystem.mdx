...
**Session si teď píšou mezi sebou, i&nbsp;napříč stroji.** `SendMessage` najde jinou session Claude Code podle jména — na macOS a&nbsp;Linuxu i&nbsp;session běžící na jiném stroji, ne jen ty ve stejném seznamu `claude agents`. Napiš do promptu `@` a&nbsp;jméno session — stejná zkratka jako `@Explore` — a&nbsp;Claude ji osloví rovnou přes `SendMessage`, aniž bys musel volat nástroj sám. `ListAgents` označí session z&nbsp;jiného stroje jako `name [ref]`, takže se ti nesplete se stejnojmennou lokální. Session teď může konverzaci začít sama, ne jen odpovídat, až se jí někdo ozve první. `crossSessionInbound` určuje, jak moc session s&nbsp;obejitými oprávněními věří příchozí zprávě — zprávy, které jí přijdou, se drží na schválení, zatímco zprávy do session s&nbsp;běžnými oprávněními se doručí rovnou. `dialogExpiry` jen nastavuje, jak dlouho zůstane otevřený dialog čekat, než se zpráva zahodí — na důvěru vliv nemá.
...
**Forkování.** Subagent typu `subagent_type: "fork"` teď zdědí celou konverzaci i prompt cache místo toho, aby startoval od nuly — sáhni po něm, když subagent potřebuje vše, na čem jste se s Claude už shodli, ne jen stručné zadání.
