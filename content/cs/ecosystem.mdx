...
**Session si teď umí povídat mezi sebou.** `SendMessage` doručí zprávu do libovolné Claude Code session na kterémkoliv z tvých strojů — macOS a Linux — a `ListAgents` ti nejdřív ukáže, co běží. Zpráva mířící do session s obejitými oprávněními čeká na tvoje schválení (`crossSessionInbound`); všechno ostatní se doručí automaticky.
...
Instalace rovnou ze zipu funguje taky. Plugin source `archive` stáhne plugin přes HTTPS — bez gitu, bez npm — s volitelným SHA-256 pinováním, jestli chceš zamknout přesně ty byty, kterým věříš.
...
**Self-hosted, jestli máš Team nebo Enterprise.** `claude self-hosted-runner` udělá z tvých vlastních strojů nebo kontejnerů místo, kde běží stejné web, mobilní a desktopové session.
