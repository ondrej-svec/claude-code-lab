...
Pro opakující se delegování — třeba reviewera, co vždycky aplikuje vaše týmové standardy — požádej Claude:

<Prompt>Vytvoř subagenta code-reviewer, který před každým commitem zkontroluje diff podle našich týmových konvencí.</Prompt>

Claude vygeneruje definici a uloží ji do `.claude/agents/my-agent.md`. Dostupný přes `@my-agent` nebo přirozeným jazykem. Agenty upravíš nebo smažeš přímou editací `.md` souborů.
