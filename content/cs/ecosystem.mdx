...
**Skládání invokací.** Zadej `/skill-a /skill-b udělej XYZ` a všechny vedoucí skilly se načtou — nejvýš pět. Hodí se, když úloha skutečně potřebuje dvě sady skillů naráz.
...
Můžeš je volat přímo, nebo to nech Claude rozhodnout. Subagenti teď standardně běží na pozadí — hlavní konverzace pokračuje dál, zatímco pracují, a Claude dostane oznámení, až skončí.

### Zkus jednoho: vestavěný Explore agent

Vestavěný. Read-only. Běží na modelu tvé session (max Opus) místo pevného Haiku — ideální na otázky „kde je X?“ ve velkém codebase.
...
Pro opakující se delegování — třeba reviewera, co vždycky aplikuje týmové standardy — popros Claude přímo nebo vlož `.md` soubor do `.claude/agents/`:

<Prompt>Vytvoř subagenta code-reviewer pro tento projekt. Čte diff a aplikuje naše standardy kódování.</Prompt>

Agent přistane v `.claude/agents/code-reviewer.md` a je dostupný přes `@code-reviewer` nebo přirozeným jazykem.
