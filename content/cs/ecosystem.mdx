...
Vestavěný. Read-only. Jede na modelu tvé session (s limitem Opus) místo Haiku. Ideální na otázky „kde je X?“ ve velkém codebase.
...
Pro opakující se delegování — třeba reviewera, co vždycky aplikuje vaše týmové standardy — řekni to Claudovi přímo nebo edituj `.claude/agents/` sám. Agent žije jako `.md` soubor a je dostupný přes `@my-agent` nebo přirozeným jazykem.
