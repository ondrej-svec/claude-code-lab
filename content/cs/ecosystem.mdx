...
**Ochrana nedůvěryhodných workspace.** `claude mcp list` a `claude mcp get` teď nespouštějí `.mcp.json` servery, které si repo samo schválilo přes committed `.claude/settings.json` — v nedůvěryhodných workspace se takové servery zobrazí jako `⏸ Pending approval`.
