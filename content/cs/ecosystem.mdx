...
**Skládání skillů.** `/skill-a /skill-b udělej XYZ` načte všechny zadané skilly ještě před tím, než Claude uvidí prompt — až pět naráz. Hodí se, když úkol zasahuje do dvou oblastí bez přepínání mezi nimi.
...
Můžeš je volat přímo, nebo to nech Claude rozhodnout. Subagenti běží standardně na pozadí — Claude pokračuje v hlavním úkolu, zatímco subagent pracuje paralelně, a výsledek si zvedne, až bude hotový.

### Zkus jednoho: vestavěný Explore agent

Vestavěný. Read-only. Dědí model hlavní session, nejvýše Opus — pevná vazba na Haiku odpadla. Ideální na otázky „kde je X?" ve velkém codebase.
...
Pro opakující se delegování — třeba reviewera, co vždycky aplikuje vaše týmové standardy — požádej Claude přímo:

> Vytvoř projektového agenta, který bude reviewovat kód podle našich TypeScript konvencí.

Claude zapíše definici do `.claude/agents/my-agent.md`. Uprav soubor podle potřeby — omez nástroje, napiš systémový prompt, změň popis. Agent je dostupný přes `@my-agent` nebo přirozeným jazykem hned v dalším kole.
