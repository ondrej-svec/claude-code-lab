...
V&nbsp;jedné invokaci nakupíš až pět skillů — `/skill-a /skill-b udělej XYZ` načte všechny skilly uvedené na začátku, ještě před spuštěním příkazu.
...
Subagenti teď standardně běží na pozadí — Claude pokračuje v hlavním úkolu a dostane zprávu, až jsou hotovi. Agenti v `claude agents`, kteří dokončí kód ve worktree, commitnou, pushnou a otevřou draft PR sami od sebe — bez ptání.

### Zkus jednoho: vestavěný Explore agent

Vestavěný. Read-only, ideální na otázky „kde je X?" ve velkém codebase. Přebírá model hlavní session (maximálně Opus) — pevné Haiku je pryč.
