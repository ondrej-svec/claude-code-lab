...
## Cache, kterou nevidíš

Compact, clear a resume jsou tahy, co děláš sám. Prompt cache běží pod kapotou při každém kole, ať o tom přemýšlíš nebo ne — a teď si můžeš nastavit, jak dlouho vydrží.

`promptCacheTtl` a `subagentPromptCacheTtl` dovolí session s API klíčem nebo na cloud providerech držet hodinovou cache na hlavní konverzaci, zatímco subagenti zůstanou na pěti minutách. `experimental.cacheTtl` (`"5m"` nebo `"1h"`) ve frontmatteru agenta přepíše TTL jen pro toho jednoho agenta, pokud není nic nastavené na úrovni subagenta.

Delší TTL se hodí na session s dlouhými pauzami mezi koly — čekáš na build, sedíš nad rozhodnutím — kde by krátké okno cache jinak vynutilo při dalším promptu drahé znovunačtení celého kontextu.
...
- **Záchrana „uklidil jsem moc rychle“.** Vyčistil jsi to, začal na čisto, narazil jsi do zdi, došlo ti, že odpověď byla v té předchozí. Nejkratší cesta: `/rewind` — dostane se i před `/clear` a konverzaci obnoví přímo. Delší cesta: `/resume`, zkopíruj relevantní kolo, znovu `/clear`, vlož to jako kontext.
