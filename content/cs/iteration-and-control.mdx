...
**Události background agentů.** Background session v `claude agents` teď volají `Notification` hook při dvou nových událostech: `agent_needs_input` (session čeká na&nbsp;tebe) a `agent_completed` (session skončila). Připoj na libovolný kanál — Slack webhook, terminálový zvonek, systémové upozornění.
