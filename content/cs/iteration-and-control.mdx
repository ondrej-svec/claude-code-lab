...
2. **Accept edits** — Claude edituje volně, diff si prohlédneš až po. Nově se ptá před zápisem do konfiguračních souborů shellu (`.zshenv`, `.zlogin`, `.bash_login`, `~/.config/git/`) a&nbsp;build-tool configů (`.npmrc`, `.yarnrc*`, `.bazelrc`, `.devcontainer/` apod.), které umožňují spuštění kódu.
