...
2. **Accept edits** — Claude edituje volně, diff si prohlédneš až po. Výjimka: build-tool konfigurační soubory, které umožňují spouštění kódu (`.npmrc`, `.yarnrc*`, `bunfig.toml`, `.bazelrc`, `.pre-commit-config.yaml`, `.devcontainer/` a podobné), se i v tomhle režimu ptají.
