...
**Hook `DirectoryAdded`.** Spustí se po `/add-dir`, nebo když SDK zavolá `register_repo_root` a zaregistruje nový pracovní adresář uprostřed session. Použij ho na to, aby se setup — instalace hooků, načtení vnořeného CLAUDE.md, zahřátí cache — spustil hned, jak přibude nový root, ne až při dalším restartu.
