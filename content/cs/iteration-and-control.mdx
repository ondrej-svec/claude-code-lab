...
**Notifikace background agentů.** Na `Notification` hooku se spouštějí dvě nové události: `agent_needs_input` (background session čeká na tvoji odpověď) a `agent_completed` (skončila). Nadrátuj je na desktopový notifikační skript a nemusíš ručně hlídat `claude agents`.
