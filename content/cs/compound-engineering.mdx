...
<Screenshot
  src="/screenshots/ch6-compound-loop.png"
  alt="Pětikroková compounding smyčka — brainstorm, plan, work, review, compound — s artefakty, které se hromadí uprostřed"
  caption="Každý průchod zanechá dokumenty — plány, diffy, revize, skilly — ze kterých čerpá další brainstorm a plán."
/>
...
## Background agenti a draft PR

Od verze 2.1.198 background agenti spuštění z `claude agents`, kteří dokončí kódovou práci v worktree, sami commitují, pushují a otevírají draft PR — místo toho, aby se zastavili a ptali.
...
1. Vyber si malou, reálnou featuru — na ukázkách třeba rate limiting na `/api/items`, request logging, nebo jednoduchý header-based auth check. Na vlastním repu něco, kolem čeho chodíš a co stihneš za 30 minut. Ne víkendový projekt.
2. Pusť smyčku, všech pět příkazů po sobě. Nejdřív `/brainstorm` — nech ho problém rozšířit. Pak `/plan`; plán si přečti, než ho potvrdíš. Pak `/work`. Pak `/review` proti plánu. Pak `/compound`, ať po sobě necháš dokument.
3. Nevynechávej `/compound` jen proto, že úkol byl malý. Ten dokument je pointa — díky němu bude tvůj příští `/brainstorm` rychlejší.

Když to funguje: tvoje ukázka má featuru postavenou a reviewnutou tím správným způsobem, a v `docs/solutions/` máš jeden dokument, který tam nebyl. Praxe v další kapitole k tomu přidá testy, security review a volitelně CI.
