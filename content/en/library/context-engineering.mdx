...
**Active context** is what's in the current conversation. Token-heavy, expensive to keep, perfectly fresh. Every turn lives here. API-key and cloud-provider sessions can keep that layer warm longer: `promptCacheTtl` sets it up to 1 hour for the main conversation, `subagentPromptCacheTtl` up to 5 minutes (the default) for subagents.
