...
**API-key and cloud-provider users get a lever here too.** `promptCacheTtl` keeps the main conversation's prompt cache alive for an hour instead of the default five minutes; `subagentPromptCacheTtl` controls the same thing for subagent calls, which stay at five minutes by default. A longer TTL on active context means fewer full-price re-reads when you step away and come back — direct savings on the layer that costs the most.
