...
<Screenshot
  ...
  alt="Diagram of three memory layers flowing left to right: Active (token-heavy, fresh) → Summarized (compact output, lossy) → Externalized (CLAUDE.md, plan docs, durable). A token-cost tick line above the row decreases left to right. Active is rose-accented, externalized is teal-accented."
  caption="Three layers of context. The skill is moving things down a layer before they cost you."
/>
...
## The cache you don't see

Compact, clear, and resume are moves you make. Prompt caching happens underneath every turn whether you think about it or not — and now you can tune how long it holds.

`promptCacheTtl` and `subagentPromptCacheTtl` let API-key and cloud-provider sessions keep a 1-hour prompt cache on the main conversation while subagents stay at 5 minutes. Set `experimental.cacheTtl` (`"5m"` or `"1h"`) in an agent's own frontmatter to override the TTL for that one agent when no subagent-level setting applies.

Reach for the longer TTL on sessions with long gaps between turns — a build you're waiting on, a decision you're sitting with — where a short cache window would otherwise force a full, expensive re-read of everything in context the next time you type.
...
<TerminalOutput
  ...
  alt="Terminal still showing /usage at 84% context (168k / 200k), then a focused /compact run, then /usage again at 29% (58k / 200k) with the summary as the heaviest contributor"
  caption="Same conversation. Less context. Same shape."
/>
...
- **The "I cleared too eagerly" recovery.** Cleared, started fresh, hit a wall, realized the answer was already in the previous conversation. Shortest path: `/rewind` — it now reaches back before a `/clear` and restores the conversation directly. Longer path: `/resume`, copy the relevant turn, `/clear` again, paste it as context.
