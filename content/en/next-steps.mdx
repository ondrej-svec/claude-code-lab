...
<Prompt>Write a failing end-to-end test for a POST /api/items endpoint that creates an item and returns 201. I haven't built the endpoint yet — just get the test to fail on the right line.</Prompt>
...
<Prompt>Write a failing test for slugify(input: string) handling diacritics and consecutive spaces. Use our existing test setup.</Prompt>

Then:

<Prompt>Make the test pass. Minimal implementation only.</Prompt>

Then:

<Prompt>Refactor. Keep the tests green. Look for duplication or unclear naming.</Prompt>
...
`/review` is now an alias — same command. Point `/code-review` at a specific pull request with `/code-review <level> <pr#>` instead of reviewing your working diff, or reach for `/code-review ultra` when the change is high-stakes enough to justify a deep cloud review.
...
- `sandbox.failIfUnavailable: true` — Claude Code refuses to run if the sandbox can't start. No silent fallback.
- `allowUnsandboxedCommands: false` — disables the escape hatch that lets Claude re-run a failing command unsandboxed.
- `sandbox.network.allowManagedDomainsOnly: true` — only admin-approved domains reach the network; users can extend the path allowlist but can't remove admin restrictions.
- `sandbox.network.strictAllowlist: true` — denies non-allowlisted hosts for sandboxed commands without prompting.
...
→ [Hooks docs](https://code.claude.com/docs/en/hooks) · chapter 9.7 for the event catalog
