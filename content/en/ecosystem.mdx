...
**Sessions can message each other now.** `SendMessage` reaches any Claude Code session on any of your machines — macOS and Linux — with `ListAgents` to see what's running first. A message aimed at a session running with bypassed permissions waits for your approval (`crossSessionInbound`); everything else auto-delivers.
...
Installing straight from a zip works too. The `archive` plugin source pulls a plugin over HTTPS — no git, no npm — with optional SHA-256 pinning if you want to lock the exact bytes you're trusting.
...
**Self-hosted, on Team and Enterprise.** `claude self-hosted-runner` turns your own machines or containers into a place these same web, mobile, and desktop sessions run.
