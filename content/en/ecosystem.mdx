...
Stack up to five skills in a single invocation — `/skill-a /skill-b do XYZ` loads all leading skills before running the command.
...
Subagents now run in the background by default — Claude keeps working on your main task while they run and is notified when they complete. Background agents in `claude agents` that finish code work in a worktree will commit, push, and open a draft PR automatically rather than stopping to ask.

### Try one: the built-in Explore agent

Ship-built. Read-only, ideal for "where is X?" questions in a large codebase. Inherits the main session's model (capped at Opus) — no longer pinned to Haiku.
