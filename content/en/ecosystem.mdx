...
A **subagent** is a specialised worker Claude spawns with its own context window, tools, and permissions. When Claude needs to do something heavy — explore a big codebase, run a review, summarise a long file — it delegates to a subagent instead of burning your main context. Subagents run in the background by default: Claude keeps working while they run and is notified when they finish.
...
Ship-built. Read-only, ideal for "where is X?" questions in a large codebase. Inherits your session model, capped at Opus — no longer pinned to Haiku.
