...
**Nested project skills.** Skills in `.claude/skills/` subdirectories inside a project load automatically when Claude works on files in that subtree. On a name clash, the nested skill appears as `<dir>:<name>` so both stay available.
