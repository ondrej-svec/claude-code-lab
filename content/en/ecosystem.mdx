...
**Stacked invocations.** `/skill-a /skill-b do XYZ` loads all leading skills (up to 5), then runs your task with all of them active. Useful when two skills naturally collaborate — a research skill and a formatting one in a single command.
...
**Auto-background on long calls.** MCP tool calls that run longer than 2 minutes move to the background automatically, keeping the session responsive. Configure the threshold or disable with `CLAUDE_CODE_MCP_AUTO_BACKGROUND_MS`.
...
Ship-built. Read-only. Inherits your session's model, capped at Opus — no longer fixed to Haiku. Ideal for "where is X?" questions in a large codebase.
...
For a recurring specialist — say, a reviewer that always applies your team's standards — ask Claude to create a subagent definition for you, or edit `.claude/agents/my-agent.md` directly. Scope it to Personal or Project, restrict its tools, save. Your agent is available via `@my-agent` or natural language.
...
**Background by default.** Subagents now run in the background — Claude keeps working while they run and is notified when they finish. A background agent doing code work in its own worktree commits, pushes, and opens a draft PR on completion instead of stopping to ask.

**`/fork` and `/subtask`.** `/fork` copies the current conversation into a new background session (its own row in `claude agents`), so you keep working in the original. `/subtask` is the new name for the previous in-session subagent that `/fork` used to launch.
