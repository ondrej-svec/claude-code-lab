...
You can invoke them directly, or let Claude choose. Subagents run in the background by default. Claude keeps working on the main task while they run and is notified when they finish.
...
For recurring delegation — say, a reviewer that always applies your team's standards — just ask Claude:

<Prompt>Create a subagent called code-reviewer that reads our codebase and reviews changes against team conventions. Restrict it to read-only tools.</Prompt>

Claude generates the definition and writes it to `.claude/agents/code-reviewer.md`. Scope it (Personal or Project), restrict its tools by editing the file, and your agent is available via `@code-reviewer` or natural language.
