...
Sessions can talk to each other now, too — on any of your machines, on macOS and Linux. `ListAgents` lists what's reachable; `SendMessage` sends a message straight to one of them. That now reaches further: `SendMessage` can start a conversation with a Remote Control session on another machine by name, instead of only replying once it's messaged you first. `ListAgents` shows that session as `name [ref]`. Handy when a background session on your laptop finds something the one on your desktop needs to know before you're back at either keyboard.
...
No git or npm access to the plugin's source? The `archive` plugin source installs straight from a zip over HTTPS, with optional SHA-256 pinning so you know exactly what you're running.
