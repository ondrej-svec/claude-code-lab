...
**Stacking skills.** `/skill-a /skill-b do XYZ` loads all leading skills before Claude sees the prompt — up to five at once. Useful when a task crosses two skill domains without switching between them.
...
You can invoke them directly, or let Claude choose. Subagents run in the background by default — Claude keeps working on your main task while a subagent runs in parallel, then picks up its result when it finishes.

### Try one: the built-in Explore agent

Ship-built. Read-only. Inherits the main session's model, capped at Opus — no longer hard-wired to Haiku. Ideal for "where is X?" questions in a large codebase.
...
For recurring delegation — say, a reviewer that always applies your team's standards — ask Claude:

> Create a project agent that reviews code for our TypeScript conventions.

Claude writes the definition to `.claude/agents/my-agent.md`. Edit the file to restrict tools, adjust the system prompt, or change the description. The agent is available via `@my-agent` or natural language immediately.
