...
**Stacked skill invocations.** Chain up to five skills before a prompt: `/skill-a /skill-b do XYZ` loads all leading skills before Claude sees the task. Useful when a task spans domains — `/security-review /code-review refactor this auth module` applies both lenses.
...
Since 2.1.198, **subagents run in the background by default** — Claude keeps working while they run and is notified when they finish.

### Try one: the built-in Explore agent

Ship-built. Read-only, inherits the session's model (capped at Opus) — ideal for "where is X?" questions in a large codebase.
...
<Prompt>Create a code-reviewer subagent scoped to this project. Apply our team review standards.</Prompt>

Claude writes the definition to `.claude/agents/my-agent.md`. Your agent is then available via `@my-agent` or natural language. Edit the file to tune its tools, scope, or instructions — it's just a markdown file.
