...
A **skill** is a bundle of instructions + optional scripts that Claude loads on demand. A domain-specific playbook — lives in `~/.claude/skills/`, activates automatically when the situation matches. Stack multiple skills in one prompt: `/skill-a /skill-b do XYZ` loads all leading skills, up to five.
...
One built-in skill worth knowing: `/dataviz` — chart and dashboard design guidance with a runnable color-palette validator. Invoke it before writing chart code in any medium.
...
**Workspace trust.** `claude mcp list` and `claude mcp get` no longer spawn `.mcp.json` servers that a repo self-approved via a committed `.claude/settings.json`. In untrusted workspaces those servers show `⏸ Pending approval` — you approve them explicitly before they connect.
...
You can invoke them directly, or let Claude choose. Since v2.1.198, subagents run in the background by default — Claude keeps working in the main session while they run, and is notified when they finish.

### Try one: the built-in Explore agent

Ship-built. Read-only, fast — inherits your session model, capped at Opus — ideal for "where is X?" questions in a large codebase.
...
For recurring delegation — say, a reviewer that always applies your team's standards — ask Claude directly:

<Prompt>Create a code-reviewer subagent scoped to this project. Read-only, no edits or shell.</Prompt>

Claude generates the definition and writes it to `.claude/agents/`. You can also edit the files in `.claude/agents/` directly.
...
**Extended thinking.** Subagents and context compaction inherit the session's extended thinking configuration, improving output quality on delegated tasks.
