...
For recurring delegation — say, a reviewer that always applies your team's standards — ask Claude to write one, or create the definition file directly:

```
claude "Create a code-reviewer subagent that enforces our team conventions"
```

Claude writes the definition to `.claude/agents/<name>.md`. Scope it (Personal or Project), restrict its tools. Your agent is then available via `@<name>` or natural language. The `/agents` creation wizard was removed in v2.1.198 — ask Claude to create or manage subagents, or edit the files in `.claude/agents/` directly.
...
**Background by default.** Subagents run in the background since v2.1.198 — Claude keeps working in your main conversation while the subagent handles its task, and is notified when it finishes.
