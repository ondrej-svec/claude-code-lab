...
Ship-built. Read-only. Runs on your session's model (capped at Opus) instead of Haiku. Ideal for "where is X?" questions in a large codebase.
...
For recurring delegation — say, a reviewer that always applies your team's standards — ask Claude to create or manage subagents, or edit `.claude/agents/` directly. Either way, your agent lives as a `.md` file and is available via `@my-agent` or natural language.
