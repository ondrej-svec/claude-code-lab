...
**Untrusted workspaces (v2.1.196+).** When you clone a repo with a committed `.claude/settings.json` that self-approves `.mcp.json` servers, those servers no longer auto-start. `claude mcp list` shows them as `⏸ Pending approval` — explicit approval required before they connect.
