...
**Stacking.** `/skill-a /skill-b do XYZ` loads all leading skills (up to 5). Combine playbooks in one invocation when a task cuts across multiple domains.
...
Subagents now run in the background by default — Claude keeps working while they run and is notified when they finish. Background agents launched from `claude agents` into a worktree go further: when the code work is done, they commit, push, and open a draft PR automatically. No need to ask.
