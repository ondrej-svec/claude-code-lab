...
**Checking connections.** `/mcp` shows the tool count for each connected server and flags any server that connected with 0 tools — zero tools means the server connected but nothing loaded. When a server fails to connect, both `claude mcp list` and `/mcp` now show the HTTP status and error text — no more guessing why a server won't come up.
...
**Nesting.** Sub-agents can now spawn their own sub-agents up to depth 3 by default (it was 1) — set `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH=1` to go back to the old no-nesting default. Fan-out that used to need a manual opt-in now chains automatically up to that depth.
