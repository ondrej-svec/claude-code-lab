...
**Skill stacking.** Multiple skills load from a single prompt: `/skill-a /skill-b do XYZ` activates both (up to five leading skills). Useful for composing a specialized skill on top of a general one without editing either.
...
Ship-built. Read-only, ideal for "where is X?" questions in a large codebase. Inherits your session's model (capped at Opus).
...
For recurring delegation — say, a reviewer that always applies your team's standards — ask Claude or edit `.claude/agents/` directly. The `/agents` creation wizard was removed in v2.1.198; natural-language creation and direct file editing are the two paths now.

<Prompt>Create a project subagent called code-reviewer that checks our team's conventions from CLAUDE.md before any code review.</Prompt>

Claude generates the agent definition and saves it. Your agent is available via `@code-reviewer` or natural language.
...
**Background by default.** Subagents now run in the background — Claude keeps working while they run and is notified when they finish. Background agents launched from the agents view (`claude agents`) when working in a worktree also auto-commit, push, and open a draft PR when they finish code work, instead of stopping to ask.
