...
**Nesting.** Sub-agents no longer spawn their own sub-agents by default — set `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` to allow deeper nesting. Fan-out that used to chain automatically from the first sub-agent now needs that opt-in.
