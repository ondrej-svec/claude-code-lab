...
**Untrusted workspace protection.** `claude mcp list` and `claude mcp get` no longer spawn `.mcp.json` servers that a repo self-approved via a committed `.claude/settings.json`; untrusted workspaces show `⏸ Pending approval`.
