...
Ship-built. Read-only, runs on the session's model (capped at Opus). Ideal for "where is X?" questions in a large codebase.
...
For recurring delegation — say, a reviewer that always applies your team's standards — ask Claude in your session or edit `.claude/agents/` directly.

<Prompt>Create an agent called "my-reviewer" that applies our team's security standards to every diff. Scope it to this project.</Prompt>

Claude writes the definition to `.claude/agents/my-reviewer.md`. Your agent is available via `@my-reviewer` or natural language.

<Screenshot
  src="/screenshots/ch5-agents-ui.png"
  alt="/agents UI listing project, personal, and library subagents with descriptions"
  caption="`/agents` lists all configured agents — agents live as `.md` files in `.claude/agents/`."
/>
