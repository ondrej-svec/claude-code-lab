...
Pin a session with `Ctrl+T` inside the `claude agents` view — pinned sessions stay alive when idle, restart automatically when Claude Code updates, and are only shed under memory pressure after non-pinned sessions.
...
Plugins can declare `defaultEnabled: false` in `plugin.json` — they stay off until you enable them via `/plugin` or `claude plugin enable`. The Discover tab surfaces plugins whose relevance signals match the current directory with a "suggested for this directory" annotation — so the right tool appears when you're in the right repo.
