...
**Stacked invocations.** Type `/skill-a /skill-b do XYZ` and all leading skills load, up to five — useful when a task genuinely needs two skill sets at once.
...
You can invoke them directly, or let Claude choose. Subagents run in the background by default — Claude keeps working while they run and is notified when they finish.

### Try one: the built-in Explore agent

Ship-built. Read-only. Runs on your session's model (capped at Opus) instead of always using Haiku — ideal for "where is X?" questions in a large codebase.
...
For recurring delegation — say, a reviewer that always applies your team's standards — ask Claude directly or drop a `.md` file in `.claude/agents/`:

<Prompt>Create a subagent called code-reviewer scoped to this project. It reads the diff and applies our coding standards.</Prompt>

Your agent lands at `.claude/agents/code-reviewer.md` and is available via `@code-reviewer` or natural language.
