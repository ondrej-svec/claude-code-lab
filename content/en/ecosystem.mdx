...
Ship-built. Read-only, inherits your session model (capped at Opus) — ideal for "where is X?" questions in a large codebase.
...
<Prompt>Create a code-reviewer subagent that applies our team conventions before any commit.</Prompt>

Claude generates the definition and saves it to `.claude/agents/my-agent.md`. Available via `@my-agent` or natural language. Edit or remove agents by editing the `.md` files directly.
