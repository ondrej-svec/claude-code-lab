...
| `/fork` | Copy the conversation into a new background session — its own row in `claude agents` — while you keep working in this one. |
...
| `/model` | Switch model for the current session (e.g., Opus → Sonnet). **Sonnet 5 is now the default model** — native 1M-token context window, promotional pricing of $2/$10 per Mtok through August 31. Requires v2.1.197+. Selecting a model now saves it as the default for new sessions. Press `s` in the picker to switch for the current session only. Set `fallbackModel` in `settings.json` to chain up to three fallback models tried in order when the primary is overloaded or unavailable. `--fallback-model` also applies to interactive sessions. |
| `/fast` | Toggle fast mode. Opus 5 is now the default Opus model — 1M context, fast mode at $10/$50 per Mtok. `/fast` applies to Opus 5 and Opus 4.8; Opus 4.7 was dropped from fast mode. |
...
| `/code-review [effort]` | Correctness review at a chosen effort level (e.g. `/code-review high`). Runs as a background subagent — review work no longer fills your conversation; stacked slash commands stay its review target. Pass `--comment` to post findings as inline GitHub PR comments. Pass `--fix` to apply findings to the working tree. Claude no longer runs `/verify` or `/code-review` on its own — invoke them explicitly when you want them. `/simplify` runs a cleanup-only pass — reuse, simplification, efficiency, altitude — not the full bug-hunting review. |
...
| `sandbox.network.strictAllowlist` | Denies non-allowlisted hosts for sandboxed commands outright — no prompt, no fallback ask. |
