...
| `/fork` | Copy the current conversation into a new background session (visible in `claude agents`) while you keep working. In-session equivalent: `/subtask`. |
...
| `/doctor` | Full setup checkup — diagnoses *and fixes* issues. Alias: `/checkup`. |
