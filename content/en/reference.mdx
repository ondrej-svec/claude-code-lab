...
| `/model` | Switch model for the current session. **Claude Sonnet 5 is now the default** (1M-token context window). Selecting a model saves it as the default for new sessions. Press `s` in the picker to switch for the current session only. Set `fallbackModel` in `settings.json` to chain up to three fallback models tried in order when the primary is overloaded or unavailable. `--fallback-model` also applies to interactive sessions. |
