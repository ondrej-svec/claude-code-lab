...
| `/fork` | Copy the current conversation into a new background session (its own row in `claude agents`) while you keep working. |
| `/subtask` | Launch a subagent in-session. |
...
| `sandbox.filesystem.disabled: true` | Skip filesystem isolation while keeping network egress control. |
