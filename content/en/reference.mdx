...
| `/model` | Switch model for the current session (e.g., Opus → Sonnet). **Sonnet 5 is now the default model** — native 1M-token context window, promotional pricing of $2/$10 per Mtok through August 31. Requires v2.1.197+. Selecting a model now saves it as the default for new sessions. Press `s` in the picker to switch for the current session only. Set `fallbackModel` in `settings.json` to chain up to three fallback models tried in order when the primary is overloaded or unavailable. `--fallback-model` also applies to interactive sessions. Within the Opus family, Opus 5 (`claude-opus-5`) is now the default model — 1M context, fast mode priced at $10/$50 per Mtok. Set `ANTHROPIC_DEFAULT_MODEL` to pin what new sessions start on — unlike `ANTHROPIC_MODEL`, a `/model` pick still overrides it and the override persists across restarts. |
...
| `/config` | Open the settings UI. `/config key=value` sets any setting from the prompt — e.g. `/config thinking=false`. `/config --help` lists all shorthand keys. A built-in "Concise" output style — results first, no preamble, same thoroughness — lives under Output style in `/config`. |
...
Set `keybindingFlavor: "readline"` in `settings.json` and `Ctrl+W` in the prompt deletes back to the previous whitespace, Bash-style, instead of the classic default. The same word keys now behave the readline/Bash way in the prompt itself: `Alt+F` and `Ctrl`/`Option+→` stop at the end of a word, `Alt+D` deletes to it (`Ctrl+Y` pastes it back), and punctuation counts as a word boundary.
