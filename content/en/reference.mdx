...
**Todo/task tools no longer ship by default on newer models.** `TaskCreate`/`TaskGet`/`TaskUpdate`/`TaskList` and `TodoWrite` are off by default on Opus 4.8, Sonnet 5, Fable 5, Mythos 5, and newer models. If you rely on watching the visible task list, set `CLAUDE_CODE_ENABLE_TODO_TOOLS=1`.
