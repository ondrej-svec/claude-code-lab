...
| `/model` | Switch model for the current session (e.g., Opus → Sonnet). Press `d` in the picker to set a default for new sessions. |
...
| `/usage`, `/stats` | Usage metrics — `/usage` shows a per-category breakdown of what's driving your limits (skills, subagents, plugins, per-MCP-server). |
...
| `/code-review [effort]` | Reports correctness bugs in the current diff at the chosen effort level (e.g., `/code-review high`). Pass `--comment` to post findings as inline GitHub PR comments. |
| `/security-review` | Bundled security review skill. |
| `/diff` | Show diff since last checkpoint. |
...
Shell commands, HTTP endpoints, or LLM prompts fired at specific lifecycle events. Events include `SessionStart`, `PreToolUse`, `PostToolUse`, `FileChanged`, `Stop`, `PreCompact`, and many more.
