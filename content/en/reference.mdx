...
| `/usage`, `/stats` | Broader usage metrics. `/usage` breaks down by category — skills, subagents, plugins, and per-MCP-server cost. |
...
| `/code-review [effort]` | Correctness bug report at a chosen effort level (e.g., `/code-review high`). Pass `--comment` to post findings as inline GitHub PR comments. |
