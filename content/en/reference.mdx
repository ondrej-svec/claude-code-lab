...
**Bash-style word editing.** Set `keybindingFlavor: "readline"` in `settings.json` and the prompt's word keys match Bash: `Ctrl+W` deletes back to the previous whitespace, `Alt+F` and `Ctrl`/`Option`+`→` jump to the end of a word, `Alt+D` deletes forward to one (`Ctrl+Y` pastes it back). The default (`"classic"`) is unchanged.
