...
**Output style.** `/config` → Output style now includes a built-in **Concise** option: Claude leads with results and skips preamble and narration, without cutting corners on the work itself.

**Auto-continue at usage limit.** Hit your claude.ai usage limit mid-session and Claude Code now resumes automatically once it resets. Turn it off via `/config` → "Continue automatically at usage limit" if you'd rather control the resume yourself.
