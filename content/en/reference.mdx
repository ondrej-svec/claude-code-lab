...
| `/review` | Alias of `/code-review` — reviews the current diff, or a specific PR via `/code-review <level> <pr#>`. |
| `/code-review [effort]` | Correctness review at a chosen effort level (e.g. `/code-review high`). No level given reuses whatever you typed last. `/code-review ultra` runs a deep cloud review. `/code-review <level> <pr#>` reviews a PR instead of the working diff. Pass `--comment` to post findings as inline GitHub PR comments. Pass `--fix` to apply findings to the working tree. Claude no longer runs `/verify` or `/code-review` on its own — invoke them explicitly when you want them. `/simplify` runs a cleanup-only pass — reuse, simplification, efficiency, altitude — not the full bug-hunting review. `/code-review` now runs as a background subagent — review no longer fills your conversation, and stacked slash commands are its review target. |
...
Credential masking now covers structured values too. New options: `extract` and `onExtractNoMatch` for structured env values, `decode: "jwt"` with `maskClaims` for JWT-aware masking, and `awsPairs`/`sigv4` for AWS SigV4 re-signing. All three need `network.tlsTerminate` on. They're honored only from user, managed, or `--settings` scope.
