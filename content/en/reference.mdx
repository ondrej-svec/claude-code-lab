...
| `/config` | Open the settings UI. `/config key=value` sets any setting from the prompt (e.g. `/config thinking=false`) — interactive, `-p`, and Remote Control. `/config --help` lists all shorthand keys. |
