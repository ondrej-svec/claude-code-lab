...
| `/model` | Switch model for the current session (e.g., Opus → Sonnet). **Sonnet 5 is now the default model** — native 1M-token context window, promotional pricing of $2/$10 per Mtok through August 31. Requires v2.1.197+. Selecting a model now saves it as the default for new sessions. Press `s` in the picker to switch for the current session only. Set `fallbackModel` in `settings.json` to chain up to three fallback models tried in order when the primary is overloaded or unavailable. `--fallback-model` also applies to interactive sessions. Within the Opus family, Opus 5 (`claude-opus-5`) is now the default model — 1M context, fast mode priced at $10/$50 per Mtok. |
...
| `/effort` | Per-session effort override — now saves your default effort level per model, so switching models keeps each one's own setting. |
...
| `/config` | Open the settings UI. `/config key=value` sets any setting from the prompt — e.g. `/config thinking=false`. `/config --help` lists all shorthand keys. Output style is set here too — the built-in **Concise** style leads with results and skips preamble and narration, without losing thoroughness. |
...
| `/usage`, `/stats` | Usage metrics. `/usage` breaks down by category — skills, subagents, plugins, per-MCP-server cost — plus a Loops breakdown (run count, total tokens, tokens per run, last run) for spotting a runaway or chatty `/loop` task. |
...
**Locked-down sessions.** Launch with `--restricted` (or set `CLAUDE_CODE_RESTRICTED=1`) and Claude Code strips out the tools that run commands or code and `WebFetch` unless you name them explicitly in `--tools`. It also confines file tools to the working directory, refuses `bypassPermissions`, and ignores user, project, and local settings files. Reach for it when you want a session that can only read and edit inside a known boundary — a review pass on an untrusted repo, a CI step you don't want reaching the network.
...
Shell commands, HTTP endpoints, or LLM prompts fired at specific lifecycle events. Events include `SessionStart`, `PreToolUse`, `PostToolUse`, `FileChanged`, `Stop`, `PreCompact`, `MessageDisplay` (transform or hide assistant message text as it is displayed), `PreModelSwitch` (block or confirm a model switch before it happens) and `PostModelSwitch` (annotate it afterward — it can't block), and many more.
