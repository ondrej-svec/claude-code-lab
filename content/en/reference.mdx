...
| `/permissions` | Manage tool allow/deny rules. Opens even while Claude is working now — rule changes apply for the rest of the current turn. |
...
| `/config` | Open the settings UI. `/config key=value` sets any setting from the prompt — e.g. `/config thinking=false`. `/config --help` lists all shorthand keys. Claude Code now resumes your session on its own when a claude.ai usage limit resets — turn it off via "Continue automatically at usage limit". |
...
| `@` | File mention — `@path/to/file` attaches a file; `@Explore` mentions a subagent; `@<session-name>` mentions another Claude session by name — Claude uses `SendMessage` to reach it directly. |
