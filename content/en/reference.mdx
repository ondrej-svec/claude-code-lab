...
| `/config [key=value]` | Open the settings UI. Pass `key=value` to set a setting directly from the prompt — e.g. `/config thinking=false`. Works in interactive, `-p`, and Remote Control. |
