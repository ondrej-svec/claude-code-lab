...
| `/code-review [effort]` | Correctness review at a chosen effort level. Pass `--comment` to post as inline PR comments; `--fix` to apply findings to the working tree. `/simplify` is an alias for `--fix`. |
