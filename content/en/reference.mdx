...
| `/fork` | Copy the conversation into a new background session — its own row in `claude agents`, now in its own git worktree — while you keep working in this one. |
...
| `mode: "mask"` (credential files, Linux/WSL) | Sandboxed commands read a sentinel copy of the credential file — the whole thing, or just the spans an `extract` regex captures — while the sandbox proxy substitutes the real value on egress. Falls back to `deny` on macOS. |
