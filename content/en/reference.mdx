...
`/sandbox` wraps bash commands in a filesystem + network sandbox enforced at the OS level (Seatbelt on macOS, bubblewrap on Linux/WSL2). [Chapter 7](/en/lab/next-steps#sandboxing) has the teaching. The flags that matter for managed deployments:
...
Chapter 6 shows the agentic skill-authoring workflow (invoke `skill-creator` or `craft-skill`, answer discovery questions, review the draft).
