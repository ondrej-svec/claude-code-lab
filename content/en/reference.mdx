...
<Callout variant="tip">`fallbackModel` in `settings.json` configures up to three fallback models tried in order when the primary model is overloaded or unavailable. `--fallback-model` sets one from the command line for interactive sessions too.</Callout>
