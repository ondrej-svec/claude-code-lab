...
**Bash-style word deletion.** Set `keybindingFlavor: "readline"` in `settings.json` and `Ctrl+W` in the prompt deletes back to the previous whitespace, the way it does in Bash, instead of the classic word-boundary behavior. A later release extended the same flavor to `Alt+F` / `Ctrl+→` (stop at the end of a word) and `Alt+D` (delete forward to it), with punctuation counted as a word boundary too.
