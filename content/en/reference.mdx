...
| `/model` | Switch model for the current session (e.g., Opus → Sonnet). **Sonnet 5 is now the default model** — native 1M-token context window, promotional pricing of $2/$10 per Mtok through August 31. Requires v2.1.197+. Selecting a model now saves it as the default for new sessions. Press `s` in the picker to switch for the current session only. Set `fallbackModel` in `settings.json` to chain up to three fallback models tried in order when the primary is overloaded or unavailable. `--fallback-model` also applies to interactive sessions. Within the Opus family, Opus 5 (`claude-opus-5`) is now the default model — 1M context, fast mode priced at $10/$50 per Mtok. Curate the picker itself with the `modelPicker` setting — an ordered, labeled list of models (any id spelling, including Vertex/Bedrock ids) that appends to or replaces the built-in lineup. |
...
**Output style.** A built-in "Concise" style leads with results and skips preamble and narration, while doing the work just as thoroughly — select it under Output style in `/config`.
...
| `/usage`, `/stats` | Usage metrics. `/usage` breaks down by category — skills, subagents, plugins, per-MCP-server cost, and loops (run count, total tokens, tokens per run, last run per `/loop` task — the fast way to spot one running away). |
