...
`claude ultrareview [target]` is a CLI subcommand — not a slash command — that runs `/ultrareview` non-interactively from CI or scripts. Prints findings to stdout; `--json` for raw output. Exits 0 on completion, 1 on failure.
...
`PostToolUse` hooks can now replace tool output via `hookSpecificOutput.updatedToolOutput` — not just observe it. Previously MCP-only; now applies to all tools.
