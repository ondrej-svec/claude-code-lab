...
| `/model` | Switch model mid-conversation (e.g., Opus → Sonnet). |
...
Control skill visibility per-skill with `skillOverrides` in your settings: `off` removes the skill from both the model and `/` autocomplete; `user-invocable-only` hides it from the model but keeps the slash command; `name-only` collapses the description.
...
Hooks now get the active effort level two ways: `effort.level` in their JSON input, and `$CLAUDE_EFFORT` in the environment (also readable in Bash tool commands). Bash subprocesses additionally receive `$CLAUDE_CODE_SESSION_ID` — the same session ID hooks already see as `session_id`.
