...
**Task tracking is opt-in on current models.** TaskCreate/TaskGet/TaskUpdate/TaskList and TodoWrite — the tools behind the visible todo list mid-session — are no longer available by default on Opus 4.8, Sonnet 5, Fable 5, Mythos 5, and newer models. Set `CLAUDE_CODE_ENABLE_TODO_TOOLS=1` if you rely on the task list and notice it's gone.
