...
| `/advisor [model]` | Pair the session with an advisor — a stronger model Claude calls at decision points (before committing to an approach, when stuck on a recurring error, before declaring done). Shorthand: `opus`, `sonnet`, `fable`. Requires v2.1.98+. Fable 5 as advisor requires v2.1.170+. Fable 5.1 (`claude-fable-5-1`) is now the default Fable model — 1M context, $10/$50 per Mtok with $0.25/Mtok cache reads. On Claude apps gateway sessions, `fable` (and `best`) still resolve to Fable 5 until your gateway supports 5.1 — pick Fable 5.1 explicitly via `/model`. `/advisor off` to clear. |
