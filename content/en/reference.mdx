...
| `/config` | Open the settings UI. `/config key=value` sets any setting from the prompt — e.g. `/config thinking=false`. `/config --help` lists all shorthand keys. Includes a built-in **Concise** output style — Claude leads with results and skips preamble and narration, same thoroughness, less scaffolding. |
