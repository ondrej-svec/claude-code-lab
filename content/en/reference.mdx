...
| `/fork` | Copy the current conversation into a new background session (visible in `claude agents`) while you keep working in the original. |
| `/subtask` | Launch an in-session subagent — the role `/fork` used to fill. |
...
| `/model` | Switch model for the current session. Claude Sonnet 5 is now the default, with a native 1M-token context window. Selecting a model saves it as your default for new sessions. Press `s` in the picker to switch for the current session only. Set `fallbackModel` in `settings.json` to chain up to three fallback models tried in order when the primary is overloaded or unavailable. `--fallback-model` also applies to interactive sessions. |
...
| `/doctor` | Full setup checkup — diagnoses and can fix issues. Alias: `/checkup`. |
...
| `Shift+Tab` | Cycle permission mode: manual → acceptEdits → plan → auto. (`manual` is the new label for what was "default".) |
