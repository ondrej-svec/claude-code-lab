...
| `/model` | Switch model for the current session. **Default is now Claude Sonnet 5** (native 1M-token context window). Selecting a model saves it as the default for new sessions; press `s` to switch for the current session only. Set `fallbackModel` in `settings.json` to chain up to three fallback models tried in order when the primary is overloaded or unavailable. `--fallback-model` also applies to interactive sessions. |
...
| `/doctor` | Full setup checkup — diagnoses config conflicts and missing setup; can apply fixes. Alias: `/checkup`. |
