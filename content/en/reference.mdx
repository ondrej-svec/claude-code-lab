...
| `/config` | Open the settings UI. `/config key=value` sets a setting from the prompt — works in interactive, `-p`, and Remote Control (e.g. `/config thinking=false`). `/config --help` lists available shorthand keys. |
