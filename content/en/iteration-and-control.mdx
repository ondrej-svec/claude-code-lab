...
2. **Accept edits** — Claude edits freely, you review the diff afterward. Exception: build-tool config files that can grant code execution (`.npmrc`, `.yarnrc*`, `bunfig.toml`, `.bazelrc`, `.pre-commit-config.yaml`, `.devcontainer/`) still prompt even here.
...
5. **Auto mode** — a classifier-driven "go fast" mode that learns safe operations and asks only on the ambiguous ones. **Prefer Auto over Bypass when you want speed** — Auto keeps guardrails, Bypass removes them. Fine-tune the classifier with `settings.autoMode.hard_deny` — rules there block unconditionally, regardless of user intent or any `allow` exceptions. Auto mode doesn't suppress `AskUserQuestion` when you or a skill explicitly relies on it.
...
**`hookSpecificOutput.additionalContext`** on `Stop` and `SubagentStop` hooks lets the hook pass a message back to Claude without the response being labeled a hook error. The turn keeps going. Use it when the hook needs to report something — active tasks, handoff notes — rather than just block or exit.
