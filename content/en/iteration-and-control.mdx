...
2. **Accept edits** — Claude edits freely, you review the diff afterward. It now prompts before writing to shell startup files (`.zshenv`, `.zlogin`, `.bash_login`, `~/.config/git/`) and build-tool config files (`.npmrc`, `.yarnrc*`, `.bazelrc`, `.devcontainer/`, etc.) that grant code execution.
