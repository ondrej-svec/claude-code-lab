...
- **`/clear`** — new conversation, old one archived (resumable via `/resume`). `/rewind` now also supports resuming from before a `/clear` was run. Use `/clear` when you've pivoted to a different area of the codebase, or the conversation has 30+ turns and is drifting.
