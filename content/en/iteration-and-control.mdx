...
**Auto mode safety blockers.** In auto mode, several destructive operations are now blocked unless you explicitly asked for them: `git reset --hard`, `git checkout -- .`, `git clean -fd`, and `git stash drop` won't run if you didn't ask to discard local work; `git commit --amend` won't run if the commit wasn't made by the agent this session; `terraform destroy`, `pulumi destroy`, and `cdk destroy` won't run unless you named the specific stack.
