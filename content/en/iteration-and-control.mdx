...
2. **Accept edits** — Claude edits freely, you review the diff afterward. One exception: build-tool config files that grant code execution (`.npmrc`, `.yarnrc*`, `bunfig.toml`, `.bazelrc`, `.pre-commit-config.yaml`, `.devcontainer/`, and similar) still prompt before writing — even in this mode.
