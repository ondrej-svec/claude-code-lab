...
**`DirectoryAdded` hook.** Fires after `/add-dir` or the SDK's `register_repo_root` control request registers a new working directory mid-session. Wire it to re-run setup — install hooks, load a nested CLAUDE.md, warm a cache — the moment a new root joins the session, instead of waiting for the next restart.
