...
**Background agent notifications.** Two new events fire on the `Notification` hook: `agent_needs_input` (a background session is waiting on your response) and `agent_completed` (it finished). Wire these to a desktop notification script and you don't have to poll `claude agents` manually.
