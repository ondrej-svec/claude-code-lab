...
**`hookSpecificOutput.additionalContext`** on `Stop` and `SubagentStop` hooks lets the hook return feedback to Claude — the session keeps going rather than the response being flagged as a hook error. Use it when a stop hook needs to hand state to Claude rather than just terminate.
