...
**Background agent events.** Background sessions in `claude agents` now fire the `Notification` hook on two events: `agent_needs_input` (session blocked, waiting for you) and `agent_completed` (session finished). Wire these into any notification sink — Slack webhook, terminal bell, system alert.
