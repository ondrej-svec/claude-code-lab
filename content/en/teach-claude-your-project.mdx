...
<Prompt>Re-read CLAUDE.md and add the three conventions we've been using this session — strict TypeScript with no any, tests next to the file they cover, database migrations as numbered SQL files. Keep each section short.</Prompt>

Then:

<Prompt>Re-read CLAUDE.md and add a **Don't** section with the rules we keep rediscovering — don't touch app/legacy/, don't install new packages without asking, don't run migrations automatically.</Prompt>
...
**Target under 200 lines per file.** Longer files consume more context and reduce adherence. Chapter 9.4 has the deeper best-practices reference.
...
<Prompt>#migrations must be reversible — write the down migration alongside the up one</Prompt>
...
1. Open your project folder in Claude — `samples/dotnet-core` (Stream A — the .NET Guide), `samples/python-react` (Stream B — the python-react Guide), or a repo of your own. Both samples ship with a starter `CLAUDE.md` that already teaches the Entry domain and the badge rules; your own repo may or may not. Either way, read what's there before touching anything.
